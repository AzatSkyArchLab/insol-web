...
class BuildingMesh {
    ...
    createMesh(building) {
        ...
        mesh.userData = {
            ...
            basePoints: outerPoints.map(p => ({ x: p.x, y: p.y }))
        };
    ...
    /**
     * Создание меша из локальных координат (для DrawTool, RectTool, ProjectImporter)
     * @param {Array} points - массив точек [{x, y}, ...]
     * @param {number} height - высота в метрах
     * @param {boolean} isResidential - жилое здание
     * @returns {THREE.Mesh|null}
     */
    createFromPoints(points, height = 9, isResidential = false) {
        if (!points || points.length < 3) {
            return null;
        }
        
        // Конвертируем в Vector2
        let outerPoints = points.map(p => new THREE.Vector2(p.x, p.y));
        outerPoints = this._cleanRing(outerPoints);
