 ...
 * SceneManager.js
 * Автономная 3D-сцена
 * Поддержка OSM и ESRI Satellite тайлов
 * ============================================
 */
...
class SceneManager {
    constructor(containerId, coordinates) {
        ...
        // Источник тайлов: 'osm' | 'esri'
        this.tileSource = 'osm';
        this.currentBounds = null;
        
        // Сдвиг тайлов (в метрах)
    ...
    /**
     * Установить источник тайлов
     * @param {string} source - 'osm' | 'esri'
     */
    setTileSource(source) {
        if (source !== 'osm' && source !== 'esri') {
            console.warn('[SceneManager] Неизвестный источник:', source);
            return;
        }
        ...
        console.log(`[SceneManager] Тайлы: ${source}`);
        
        // Перезагружаем тайлы если есть bounds
        if (this.currentBounds) {
            this.loadGroundTile(this.currentBounds);
        }
    }
    
    /**
     * Получить текущий источник тайлов
     */
    getTileSource() {
    ...
    /**
     * Установить сдвиг тайлов
     * @param {number} x - сдвиг по X в метрах
     * @param {number} y - сдвиг по Y в метрах
     */
    setTileOffset(x, y) {
        this.tileOffset.x = x;
        this.tileOffset.y = y;
        
        // Применяем сдвиг к существующим тайлам
        if (this.currentBounds) {
            this.loadGroundTile(this.currentBounds);
        }
    }
    ...
    async loadGroundTile(bounds) {
        ...
        // Собираем тайлы для покрытия области
        const tiles = this._getTilesForBounds(bounds, zoom);
        
        console.log(`[SceneManager] Загрузка ${tiles.length} тайлов (${this.tileSource}), zoom=${zoom}`);
        
        // Очищаем ground группу
        ...
        for (const tile of tiles) {
            ...
            loader.load(tileUrl, (texture) => {
                ...
                const tilePosMeters = {
                    x: (tileCenter.lon - centerLon) * 111320 * Math.cos(centerLat * Math.PI / 180) + this.tileOffset.x,
                    y: (tileCenter.lat - centerLat) * 111320 + this.tileOffset.y
                };
    ...
    getMeasurementsGroup() {
        return this.measurements;
    }
    
    clearMeasurements() {
        while (this.measurements.children.length > 0) {
            const mesh = this.measurements.children[0];
